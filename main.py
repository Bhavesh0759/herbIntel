...
# Load free open-source model (runs locally)
...
async def chat_with_bot(message: str):
    # Intent responses
    intents = {
        "hi": "Hello! 👋 How can I help you with herbs or phytochemicals today?",
        "hello": "Hey there! 🌿 Ask me anything about medicinal herbs.",
    ...
    for key in intents:
        if key in msg:
            return {"response": intents[key]}
    
    # If not an intent, use LLM to answer
    prompt = f"Answer this like a helpful herbal medicine assistant: {message}"
    reply = chatbot(prompt, max_new_tokens=100)[0]['generated_text']
